// Adapted from projects/yandecode/packages/core/src/persistence/repositories/documents.ts:
// DocumentInput/ChunkRecord gain repository/application/source to match
// RagDocumentMetadata, and CHUNK_SELECT joins those columns in from `documents`.
...
export interface DocumentInput {
  ...
  source: 'code' | 'release_document' | 'knowledge_base';
...
const CHUNK_SELECT = `SELECT c.id, c.document_id, d.path, d.repository, d.application, d.source, d.git_commit,
  c.vector_id, c.kind, c.symbol, c.identifiers, c.start_line, c.end_line, c.content, c.content_hash, c.token_count
...
export class DocumentRepository {
  ...
  replaceDocument(
    ...
    return this.state.write((db) => {
      const removedVectorIds = (
        db
          .prepare('SELECT c.vector_id AS v FROM chunks c JOIN documents d ON d.id = c.document_id WHERE d.path = ?')
      ...
      const now = nowIso();
      db.prepare(
        `INSERT INTO documents (id, path, repository, application, source, language, size_bytes, content_hash, git_commit, indexed_at, index_generation)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ).run(
        documentId,
        doc.path,
      ...
      const seq = db
        .prepare("UPDATE vector_id_seq SET next_id = next_id + ? WHERE name = 'repository' RETURNING next_id")
  ...
  embeddingsByVectorIds(ids: number[]): Map<number, Float32Array> {
    ...
    const rows = this.state.read((db) =>
      db.prepare(`SELECT vector_id, embedding FROM chunks WHERE vector_id IN (${placeholders}) AND embedding IS NOT NULL`).all(...ids),
  ...
  *iterateEmbeddings(): IterableIterator<{ vectorId: number; embedding: Float32Array }> {
  ...
  allVectorIds(): number[] {
    return (this.state.read((db) => db.prepare('SELECT vector_id AS v FROM chunks ORDER BY vector_id').all()) as { v: number }[]).map(
      (r) => r.v,
    );
  }
}
