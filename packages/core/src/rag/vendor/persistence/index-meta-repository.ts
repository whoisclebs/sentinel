// Adapted from projects/yandecode/packages/core/src/persistence/repositories/index-meta.ts:
// IndexName is a single literal ('repository') — SENTINEL has no 'memory' index.
import { nowIso } from '../ids.js';
import type { StateService } from './state-service.js';

export type IndexName = 'repository';
...
export class IndexRepository {
  ...
  getMeta(name: IndexName): VectorIndexMeta | null {
    const r = this.state.read((db) => db.prepare('SELECT * FROM vector_index_meta WHERE name = ?').get(name)) as
      | MetaRow
      | undefined;
    if (!r) return null;
    return {
