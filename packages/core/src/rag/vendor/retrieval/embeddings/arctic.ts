// Adapted from projects/yandecode/packages/retrieval/src/embeddings/arctic.ts: userCacheDir → sentinelCacheDir, YANDECODE_MODEL_HOST/YANDECODE_MODEL_DIR → SENTINEL_MODEL_HOST/SENTINEL_MODEL_DIR.
import { existsSync } from 'node:fs';
import { join } from 'node:path';
import { sentinelCacheDir } from './cache-dir.js';
import {
  AutoTokenizer,
...
// with SENTINEL_MODEL_HOST to point at the original Hugging Face Hub
...
export function resolveModelHost(envVars: NodeJS.ProcessEnv = process.env): string {
  return envVars.SENTINEL_MODEL_HOST ?? DEFAULT_MODEL_HOST;
}

// The Hub needs a `resolve/{revision}/` segment; our flat mirror layout (and
// any other plain file host pointed at via SENTINEL_MODEL_HOST) doesn't.
...
export function resolveModelCacheDir(envVars: NodeJS.ProcessEnv = process.env): string {
  return envVars.SENTINEL_MODEL_DIR ?? join(sentinelCacheDir(envVars), 'models');
}
